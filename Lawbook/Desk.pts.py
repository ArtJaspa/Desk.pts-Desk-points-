...
class DeskGui():
    ...
    def prod(self):
        with codecs.open(self.tempout, 'w') as routput:
            ...
            outputwriter.writerow(['RefId', 'ClusterId', 'Content Name', 'BatchName', 'XML Filename', 'Task', 'Process Type', 'Productivity', 'uom', 'DateTime Start', 'DateTime End', 'Spent', 'Status', 'Remarks','UserId'])
            for a in self.TITOrecs:
                ...
                outputwriter.writerow([a[0], a[11], content_name, a[1], xml_filename, initasklist_dict[str(a[7])], process_type, a[8], iniuomlist_dict[str(a[9])], a[3], a[4], a[5], inistatuslist_dict[str(a[6])], a[10], a[2]])
        return
    ...
    def get_from_db(self, curexecute):
        ...
        elif curexecute=='get_batch_x':
            self.cursor.execute('SELECT "Normalized Content Name", "Original XML Filename", "Process Type", "Content Name/Project Name" FROM dbo.tblShipmentDetail WHERE BatchName=?', (self.TITOrec[1],))
