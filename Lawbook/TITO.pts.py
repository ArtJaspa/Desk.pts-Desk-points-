...
'''Revision History
10/31/17 v1.0 1. allow multiple path for scalc in configuration
2. use file dialog widget
3. get productivity automatically, empty if output file is not found
4. deactivated edit mode temporarily, use earlier version

10/20/17 alpha rel. 3 Fix missing prompt whenever batchname is not registered

10/18/17 alpha rel. 2
1. Added edit function
2. Record TimeSpent on database
...
class TITOGui():
    def __init__(self, master):
        self.shipmenttable = 'dbo.tblShipmentDetail'
    ...
    def get_xfiletypes(self):
        inifiletypes = {'xml':'xml', '*':'all', 'user':user}
        x = 0
    ...
    def get_productivity(self):
        ...
        for a in self.f:
            taskout_file = '%s.%s' % (a,user)
            taskout_path = '%s/%s.%s' % (self.path,a,user)
    ...
    def filedialog(self, event):
        ...
        if self.is_task_OK==1:
            ...
            if self.OK['text']=='Taskin':
                ...
                self.path = '%s/%s' % (self.initask_split[2],self.f[0][:10])
                self.get_xfiletypes()
    ...
    def taskin(self):
        ...
        if is_done_OK==1:
            self.get_filename_init()
            if len(self.f) > 1 and self.tasknum!='6':
        ...
        for file in self.f:
            if curexecute=='my_ongoing':
                ...
                if self.RefIdvar.get()!='':
                    ...
                    #self.cursor.execute('SELECT RefId, BatchName, UserId, DateTimeStarted, DateTimeEnded, TimeSpent, StatusId, TaskId, Productivity, uom, Remarks, RefId from ' + self.TITOtable + ' WHERE UserId=? AND TaskId=? AND BatchName=?', (self.uservar.get(),self.taskvar.get().split(' ')[0],file))#delete if self.tasknum is OK
